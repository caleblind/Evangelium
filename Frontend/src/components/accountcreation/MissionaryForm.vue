...
export default {
  ...
  methods: {
    async submitForm() {
      ...
      const formData = {
        user: this.missionary.email,
        full_name: this.missionary.name,
        denomination: this.selectedDenomination,
        country: this.selectedCountry,
        years_of_experience: this.selectedExperience,
      };

      try {
        // Send data to the backend via POST (No CSRF token handling)
        const response = await axios.post(
          "http://127.0.0.1:8000/missionary/",
          formData,
          {
            headers: {
    ...
    resetForm() {
      ...
      this.missionary.bio = "";
      this.missionary.phone = ""; // Reset the phone number field
